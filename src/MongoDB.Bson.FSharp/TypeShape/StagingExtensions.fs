...
module Expr =
    ...
    let bindMutable (bname : string, bexpr : Expr<'T>)
        ...
        let v = Var(bname, typeof<'T>, isMutable = true)
    ...
    let lam2 (f : Expr<'T1> -> Expr<'T2> -> Expr<'S>) : Expr<'T1 -> 'T2 -> 'S> =
        let t1v = Var("t1", typeof<'T1>)
        let t2v = Var("t2", typeof<'T2>)
        Expr.Lambda(t1v,
            Expr.Lambda(t2v,
                f (cast(Expr.Var t1v))
                  (cast(Expr.Var t2v))))
