...
/// A Dangerous helper to create empty uninitialized instances
module FormatterServices =
    extern Object nativeGetUninitializedObject(Type t)

    let GetUninitializedObject (t: Type) : obj = nativeGetUninitializedObject t
...
module private TypeShapeImpl =
    ...
    type MemberInfo with
        member inline m.ContainsAttr<'Attr when 'Attr :> Attribute> (inheritAttr: bool) =
            m.GetCustomAttributes(inheritAttr)
            |> Seq.exists (function :? 'Attr -> true | _ -> false)

        member inline m.TryGetAttribute<'Attr when 'Attr :> Attribute> (inheritAttr: bool) =
            m.GetCustomAttributes(inheritAttr)
            |> Seq.tryPick (function :? 'Attr as attr -> Some attr | _ -> None)

    let activateGeneric (templateTy:Type) (typeArgs : Type[]) (args:obj[]) =
        let templateTy =
            if typeArgs.Length = 0 then templateTy
            elif templateTy.GetTypeInfo().IsGenericType |> not then invalidArg (string templateTy) "not generic."
            elif templateTy.GetTypeInfo().IsGenericTypeDefinition |> not then
    ...
    let resolveTypeShape(typ : Type) =
        if typ |> isNull then raise <| ArgumentNullException("TypeShape: System.Type cannot be null.")
        if typ.GetTypeInfo().IsGenericTypeDefinition then raise <| UnsupportedShape typ
...
module private MemberUtils =
    ...
    let isStructMember (path : MemberInfo[]) =
        path |> Array.exists (fun m -> m.DeclaringType.GetTypeInfo().IsValueType)
...
//     let ctorInfo = typeof<'T>.GetConstructor(ctorTypes)
//     let getCtorInfo () =
//         match ctorInfo with
...
module Shape =
    ...
    let (|Primitive|_|) (s:TypeShape) =
        if s.Type.GetTypeInfo().IsPrimitive then someU
        else None
    ...
    let (|Equality|_|) (s : TypeShape) =
        ...
        let rec isEqualityConstraint (stack:Type list) (t:Type) =
            if stack |> List.exists ((=) t) then true // recursive paths resolve to true always
                ...
                if t.GetTypeInfo().ContainsAttr<NoEqualityAttribute>(true) then false
                elif t.GetTypeInfo().ContainsAttr<CustomEqualityAttribute>(true) then true
                else
            ...
            elif t.GetTypeInfo().ContainsAttr<NoEqualityAttribute>(false) then false
            elif FSharpType.IsRecord(t, allMembers) then
                if t.GetTypeInfo().ContainsAttr<CustomEqualityAttribute>(true) then false
                else
    ...
    let (|Comparison|_|) (s : TypeShape) =
        ...
        let rec isComparisonConstraint (stack:Type list) (t:Type) =
            if t = typeof<IntPtr> || t = typeof<UIntPtr> then true
                ...
                if t.GetTypeInfo().ContainsAttr<NoComparisonAttribute>(true) then false
                elif t.GetTypeInfo().ContainsAttr<CustomComparisonAttribute>(true) then true
                else
                    FSharpType.GetUnionCases(t, allMembers)
                    |> Seq.collect (fun uci -> uci.GetFields())
                    |> Seq.map (fun p -> p.PropertyType)
            ...
            elif t.GetTypeInfo().ContainsAttr<NoComparisonAttribute>(false) then false
            elif FSharpType.IsRecord(t, allMembers) then
                if t.GetTypeInfo().ContainsAttr<CustomComparisonAttribute>(true) then false
                else
                    FSharpType.GetRecordFields(t, allMembers)
                    |> Seq.map (fun p -> p.PropertyType)
                    |> Seq.distinct
            ...
            else isInterfaceAssignableFrom typeof<IComparable> t
    ...
    let (|Struct|NotStruct|) (s : TypeShape) =
        if s.Type.GetTypeInfo().IsValueType then
    ...
    let (|Collection|_|) (s : TypeShape) =
        match s.Type.GetTypeInfo().GetInterface("ICollection`1") with
        | null ->
            match s.ShapeInfo with
    ...
    let (|Enumerable|_|) (s : TypeShape) =
        match s.Type.GetTypeInfo().GetInterface("IEnumerable`1") with
        | null ->
            match s.ShapeInfo with
